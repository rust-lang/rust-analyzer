...
import {
    vscodeReinstallExtension,
    vscodeReloadWindow,
    log,
    vscodeInstallExtensionFromVsix,
    assert,
    notReentrant,
} from '../util';
...
export async function ensureProperExtensionVersion(
    ...
    log.debug(
        'Current rust-analyzer nightly was downloaded',
    ...
    if (
        !(await askToDownloadProperExtensionVersion(
            config,
            'The installed nightly version is most likely outdated. ',
        ))
    ) {
        return;
    }
    ...
    await tryDownloadNightlyExtension(config, state, releaseInfo => {
        assert(
            currentExtReleaseDate.getTime() ===
                state.installedNightlyExtensionReleaseDate.get()?.getTime(),
...
async function askToDownloadProperExtensionVersion(config: Config, reason = ''): Promise<boolean> {
    ...
    const stableOrNightly =
        config.updatesChannel === UpdatesChannel.Stable ? 'stable' : 'latest nightly';
 ...
 */
const tryDownloadNightlyExtension = notReentrant(
    async (
        config: Config,
        state: PersistentState,
