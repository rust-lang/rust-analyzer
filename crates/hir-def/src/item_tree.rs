...
pub(crate) fn file_item_tree_query(
    ...
    let ctx = lower::Ctx::new(krate.cfg_options(db), file_id, db);
            ...
            ast::SourceFile(file) => {
                ...
                let extra_top_attrs = (file_id == root_file_id).then(|| {
                    parse_extra_crate_attrs(db, krate).map(|crate_attrs| {
                        lower_extra_crate_attrs(db, crate_attrs, root_file_id, &|| ctx.cfg_options())
                    })
                }).flatten();
...
pub(crate) fn block_item_tree_query(db: &dyn DefDatabase, block: BlockId) -> Arc<ItemTree> {
    ...
    let ctx = lower::Ctx::new(loc.module.krate(db).cfg_options(db), loc.ast_id.file_id, db);
