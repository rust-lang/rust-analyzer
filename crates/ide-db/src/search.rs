...
use hir::{
    ...
    HasSource, HirFileId, HirFileIdExt, HirFileRange, InFile, InFileWrapper, InRealFile,
            ...
            for offset in Self::match_indices(&text, finder, search_range) {
                let ret = tree.token_at_offset(offset).any(|token| {
                    ...
                    if let Some((range, Some(nameres))) =
                        sema.check_for_format_args_template_with_file(token, offset)
                    {
