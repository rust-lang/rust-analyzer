...
impl SearchScope {
    ...
    fn crate_graph(db: &RootDatabase) -> SearchScope {
        ...
        for &krate in all_crates.iter() {
            ...
            entries.extend(
                source_root
                    .iter()
                    .map(|id| (EditionedFileId::from_file_id(db, id, crate_data.edition), None)),
            );
        }
        SearchScope { entries }
    }
    ...
    fn reverse_dependencies(db: &RootDatabase, of: hir::Crate) -> SearchScope {
        ...
        for rev_dep in of.transitive_reverse_dependencies(db) {
            ...
            entries.extend(
                source_root
                    .iter()
                    .map(|id| (EditionedFileId::from_file_id(db, id, rev_dep.edition(db)), None)),
            );
        }
        SearchScope { entries }
    }
    ...
    fn krate(db: &RootDatabase, of: hir::Crate) -> SearchScope {
        ...
        SearchScope {
            entries: source_root
                .iter()
                .map(|id| (EditionedFileId::from_file_id(db, id, of.edition(db)), None))
                .collect(),
        }
    }
