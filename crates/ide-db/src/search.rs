...
use hir::{
    sym, Adt, AsAssocItem, DefWithBody, FileRange, HasAttrs, HasContainer, HasSource, HirFileId,
    HirFileIdExt, HirFileRange, InFile, InFileWrapper, InRealFile, InlineAsmOperand, ItemContainer,
...
impl SearchScope {
    ...
    fn crate_graph(db: &RootDatabase) -> SearchScope {
        ...
        for krate in graph.iter() {
            ...
            entries.extend(
                source_root
                    .iter()
                    .map(|id| (EditionedFileId::new(id, graph[krate].edition).into(), None)),
            );
        }
        SearchScope { entries }
    }
    ...
    fn reverse_dependencies(db: &RootDatabase, of: hir::Crate) -> SearchScope {
        ...
        for rev_dep in of.transitive_reverse_dependencies(db) {
            ...
            entries.extend(
                source_root
                    .iter()
                    .map(|id| (EditionedFileId::new(id, rev_dep.edition(db)).into(), None)),
            );
        }
        SearchScope { entries }
    }
    ...
    fn krate(db: &RootDatabase, of: hir::Crate) -> SearchScope {
        ...
        SearchScope {
            entries: source_root
                .iter()
                .map(|id| (EditionedFileId::new(id, of.edition(db)).into(), None))
                .collect(),
        }
    }
...
impl<'a> FindUsages<'a> {
    ...
    fn scope_files<'b>(
        ...
        scope.entries.iter().map(|(&file_id, &search_range)| match file_id.repr() {
            span::HirFileIdRepr::FileId(editioned_file_id) => {
                let text = db.file_text(editioned_file_id.file_id());
                let search_range =
        ...
        tracing::info_span!("Self_search").in_scope(|| {
            search(
                ...
                is_possibly_self.into_iter().map(|position| {
                    (
                        match position.file_id.repr() {
                            span::HirFileIdRepr::FileId(editioned_file_id) => {
                                self.sema.db.file_text(editioned_file_id.file_id())
                            }

                            span::HirFileIdRepr::MacroFile(macro_file_id) => Arc::from(
                                self.sema
                                    .db
    ...
    pub fn search(&self, sink: &mut dyn FnMut(HirFileId, FileReference) -> bool) {
        ...
        for (text, file_id, search_range) in Self::scope_files(sema.db, &search_scope) {
            ...
            for offset in Self::match_indices(&text, finder, search_range) {
                let ret = tree.token_at_offset(offset).any(|token| {
                    ...
                    if let Some((range, str_token, Some(nameres))) = sema
        ...
        match self.def {
            Definition::Module(module) if self.search_self_mod => {
                ...
                let (t1_, t2_);
                let text = match src.file_id.repr() {
                    span::HirFileIdRepr::FileId(editioned_file_id) => {
                        t1_ = sema.db.file_text(editioned_file_id.file_id());
                        &*t1_
                    }
