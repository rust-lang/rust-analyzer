...
impl db::RootDatabase {
    pub(crate) fn apply_change(&mut self, change: AnalysisChange) {
        ...
        if !change.new_roots.is_empty() {
            ...
            for (root_id, is_local) in change.new_roots {
                self.query_mut(ra_db::SourceRootQuery).set(root_id, Default::default());
                if is_local {
                    local_roots.push(root_id);
                }
            }
            self.query_mut(ra_db::LocalRootsQuery).set((), Arc::new(local_roots));
        }
        ...
        if !change.libraries_added.is_empty() {
            ...
            for library in change.libraries_added {
                ...
                self.query_mut(ra_db::SourceRootQuery).set(library.root_id, Default::default());
                self.query_mut(LibrarySymbolsQuery)
                    .set_constant(library.root_id, Arc::new(library.symbol_index));
                self.apply_root_change(library.root_id, library.root_change);
            }
            self.query_mut(ra_db::LibraryRootsQuery).set((), Arc::new(libraries));
        }
    ...
    fn apply_root_change(&mut self, root_id: SourceRootId, root_change: RootChange) {
        ...
        for add_file in root_change.added {
            self.query_mut(ra_db::FileTextQuery).set(add_file.file_id, add_file.text);
            self.query_mut(ra_db::FileRelativePathQuery)
                .set(add_file.file_id, add_file.path.clone());
            self.query_mut(ra_db::FileSourceRootQuery).set(add_file.file_id, root_id);
            source_root.files.insert(add_file.path, add_file.file_id);
        }
        for remove_file in root_change.removed {
            self.query_mut(ra_db::FileTextQuery).set(remove_file.file_id, Default::default());
            source_root.files.remove(&remove_file.path);
        }
        self.query_mut(ra_db::SourceRootQuery).set(root_id, Arc::new(source_root));
    }
    ...
    fn gc_syntax_trees(&mut self) {
        self.query(ra_db::SourceFileQuery).sweep(salsa::SweepStrategy::default().discard_values());
        self.query(hir::db::FileItemsQuery).sweep(salsa::SweepStrategy::default().discard_values());
        self.query(hir::db::FileItemQuery).sweep(salsa::SweepStrategy::default().discard_values());
    }
}
