...
use hir::{DoNotComplete, Name, PathCandidateCallback, ScopeDef, sym};
    ...
    fn on_trait_item(&mut self, item: hir::AssocItem) -> ControlFlow<()> {
        ...
        if item.container_trait(self.ctx.db).is_none_or(|trait_| {
            !self.ctx.exclude_traits.contains(&trait_)
                && trait_.do_not_complete(self.ctx.db) != DoNotComplete::Methods
...
pub(crate) fn complete_expr_path(
    ...
    match qualified {
        ...
        Qualified::TypeAnchor { ty: None, trait_: None } => ctx
            ...
            .map(hir::Trait::from)
            .filter(|it| {
                !ctx.exclude_traits.contains(it)
                    && it.do_not_complete(ctx.db) != DoNotComplete::Methods
            })
