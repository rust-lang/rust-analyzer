...
fn suggest_receiver_name(
    ...
    let placeholder = |name| SmolStr::from_iter(["${", n, ":", name, "}"]);
    ...
    let name = NameGenerator::new_with_names([].into_iter()).for_variable(receiver, sema);
    if name == "var_name" { SmolStr::from_iter(["$", n]) } else { placeholder(&name) }
}
