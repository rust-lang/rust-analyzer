                ...
                for progress in prime_caches_progress {
                    match progress {
                        ...
                        PrimeCachesProgress::End { cancelled, defer_workspace_flycheck } => {
                            ...
                            } else if defer_workspace_flycheck
                                && self.config.check_on_save(None)
                                && self.config.flycheck_workspace(None)
                                && !self.fetch_build_data_queue.op_requested()
                            {
                                // Priming finished; now run the deferred
                                // initial workspace flycheck (kept off the
                                // critical path so `cargo check` doesn't
                                // contend with cache priming for CPU).
                                self.flycheck
                                    .iter()
