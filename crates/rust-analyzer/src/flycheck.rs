...
fn test_substitutions() {
    ...
    assert_eq!(test_substitute(None, Some(saved_file), "{label} {saved_file}").as_deref(), None);
    assert_eq!(
    ...
    assert_eq!(test_substitute(Some(label), Some(saved_file), "xxx").as_deref(), Some("build xxx"));
    ...
    fn test_substitute(
        ...
        Substitutions { label, saved_file }
            ...
            .map(|command| command.get_args().map(|x| x.to_string_lossy()).collect_vec().join(" "))
