...
use crate::{
    ...
    db::HirDatabase,
    next_solver::{
        Clause, DbInterner, EarlyBinder, GenericArgs, ParamEnv, TraitRef, Ty, TyKind,
        fold::fold_tys, generics::Generics,
    },
};
...
pub fn predicates<'db>(
    ...
    let generic_params = GenericParams::new(db, loc.adt.into());
    let interner = DbInterner::new_with(db, loc.module(db).krate());
    ...
    match loc.trait_ {
        ...
        BuiltinDeriveImplTrait::Default => {
            if matches!(loc.adt, AdtId::EnumId(_)) {
                ...
                GenericPredicates::from_explicit_own_predicates(EarlyBinder::bind(
                    adt_predicates.explicit_predicates().iter_identity_copied().collect(),
                ))
            } else {
                simple_trait_predicates(interner, loc, &generic_params, adt_predicates, trait_id)
            }
        ...
        BuiltinDeriveImplTrait::CoerceUnsized | BuiltinDeriveImplTrait::DispatchFromDyn => {
            let Some((pointee_param_idx, pointee_param_id, new_param_ty)) =
                ...
                // Malformed derive.
                return GenericPredicates::from_explicit_own_predicates(EarlyBinder::bind(
                    Box::default(),
                ));
            };
...
fn simple_trait_predicates<'db>(
    ...
    adt_predicates: &GenericPredicates<'db>,
    trait_id: TraitId,
) -> GenericPredicates<'db> {
...
fn extend_assoc_type_bounds<'db>(
    ...
    fields: &ArenaMap<LocalFieldId, EarlyBinder<'db, Ty<'db>>>,
    trait_: TraitId,
) {
    ...
    for (_, field) in fields.iter() {
        field.instantiate_identity().visit_with(&mut visitor);
    }
}
