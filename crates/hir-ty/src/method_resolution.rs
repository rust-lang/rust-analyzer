...
use crate::{
    AdtId, Canonical, CanonicalVarKinds, Const, DebruijnIndex, DynTyExt, ForeignDefId,
    ...
    db::HirDatabase,
    from_chalk_trait_id, from_foreign_def_id, from_placeholder_idx,
    infer::{Adjust, Adjustment, OverloadedDeref, PointerCast, unify::InferenceTable},
    lang_items::is_box,
    next_solver::{
        self, SolverDefId, fulfill::FulfillmentCtxt, infer::DefineOpaqueTypes,
...
pub(crate) fn lookup_impl_method_query(
    ...
    struct OpaqueTyResolver<'db> {
        db: &'db dyn HirDatabase,
        subst: &'db Substitution,
        generics: crate::generics::Generics,
    }

    impl<'db> TypeFolder<Interner> for OpaqueTyResolver<'db> {
        ...
        fn fold_ty(&mut self, ty: Ty, outer_binder: DebruijnIndex) -> Ty {
            match ty.kind(Interner) {
                ...
                | TyKind::Alias(chalk_ir::AliasTy::Opaque(chalk_ir::OpaqueTy {
                    ...
                    let impl_trait_id = self.db.lookup_intern_impl_trait_id((*opaque_ty_id).into());
                    match impl_trait_id {
                        crate::ImplTraitId::ReturnTypeImplTrait(func, idx) => {
                            let infer = self.db.infer(func.into());
                            let folder = &mut OpaqueTyResolver {
                                db: self.db,
                                subst: substitution,
                                generics: crate::generics::generics(self.db, func.into()),
                            };
                            folder.fold_ty(infer.type_of_rpit[idx].clone(), outer_binder)
                        }
                        _ => ty,
                    }
                }
                _ => ty.super_fold_with(TypeFolder::as_dyn(self), outer_binder),
            }
        }
        ...
        fn fold_free_placeholder_const(
            ...
            let it = from_placeholder_idx(self.db, universe);
            self.generics
                .type_or_const_param_idx(it.0)
                .and_then(|idx| {
                    self.subst
                        .as_slice(Interner)
                        .get(idx)
                        .and_then(|arg| arg.constant(Interner).cloned())
                })
                .unwrap_or_else(|| {
                    universe.to_const(
        ...
        fn fold_free_placeholder_ty(
            ...
            self.generics
                .type_or_const_param_idx(it.0)
                .and_then(|idx| {
                    self.subst.as_slice(Interner).get(idx).and_then(|arg| arg.ty(Interner).cloned())
                })
                .unwrap_or_else(|| universe.to_ty(TypeFolder::interner(self)))
        }
    }

    let fn_subst = fn_subst
        .clone()
        .try_fold_with(
            &mut OpaqueTyResolver {
                db,
                subst: &fn_subst,
                generics: crate::generics::generics(db, func.into()),
            },
...
fn iterate_method_candidates_for_self_ty<'db>(
    ...
    let mut table = InferenceTable::new(db, env);
    let self_ty = ChalkToNextSolver::from_nextsolver(
        table.instantiate_canonical_ns(*self_ty),
        table.interner,
    );
    iterate_inherent_methods(
        &self_ty,
...
fn generic_implements_goal_ns<'db>(
    ...
    let trait_ref = rustc_type_ir::TraitRef::new_from_args(
        table.infer_ctxt.interner,
        SolverDefId::TraitId(trait_),
        args,
    )
    .with_self_ty(table.infer_ctxt.interner, self_ty);
...
fn autoderef_method_receiver<'db>(
    ...
    let mut autoderef = autoderef::Autoderef::new_no_tracking(
        table,
        ChalkToNextSolver::from_nextsolver(ty, interner),
        false,
        true,
    );
