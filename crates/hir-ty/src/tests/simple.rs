...
"#,
        expect![[r#"
            ...
            242..280 '{     ...))); }': ()
            252..253 't': &'? i32
            256..262 'A::foo': fn foo<i32>(&'?0.0 A<i32>) -> &'?0.0 i32
...
fn infer_type_alias() {
    check_infer(
        ...
        expect![[r#"
            ...
            168..171 'x.y': i128
            177..178 'y': A<&'_ str, u128>
            177..180 'y.x': &'_ str
            186..187 'y': A<&'_ str, u128>
            186..189 'y.y': u128
            195..196 'z': A<u8, i8>
            195..198 'z.x': u8
            204..205 'z': A<u8, i8>
...
fn infer_type_param() {
    check_infer(
        ...
        expect![[r#"
            ...
            105..106 'y': u32
            117..118 'x': bool
            127..132 'clone': fn clone<bool>(&'?0.0 bool) -> bool
...
fn test(mut cx: Context<'_>) {
    ...
    let result = Pin::new(&mut generator).poll_next(&mut cx);
        ...
        expect![[r#"
            ...
            228..235 '&mut cx': &'? mut Context<'_>
            233..235 'cx': Context<'_>
        "#]],
    );
}
...
fn test(mut cx: Context<'_>) {
    ...
    let result = Pin::new(&mut generator).poll_next(&mut cx);
        ...
        expect![[r#"
            ...
            237..244 '&mut cx': &'? mut Context<'_>
            242..244 'cx': Context<'_>
        "#]],
    );
}
...
"#,
        expect![[r#"
            ...
            99..100 't': Ark<T>
            110..144 '{     ... (); }': ()
            116..124 'Ark::foo': fn foo<T>(&'?0.0 Ark<T>) -> *const T
...
fn foo() {
    ...
    "#,
        expect![[r#"
            ...
            234..285 '{     ...CK); }': ()
            244..245 '_': &'? [u32; 0]
            248..263 'LazyLock::force': fn force<[u32; 0]>(&'?0.0 LazyLock<[u32; 0]>) -> &'?0.0 [u32; 0]
