...
/// Runs `f` on a thread with a stack using DEFAULT_STACK_SIZE.
/// libtest uses a smaller stack size by default.
///
/// Using a separate thread means we don't get data for cov_mark, so
/// only use this when necessary to avoid stack overflows in tests.
#[track_caller]
fn with_big_stack(f: impl FnOnce() + Send + 'static) {
    match std::thread::Builder::new()
        .name("hir-ty-tests-regressions".to_owned())
