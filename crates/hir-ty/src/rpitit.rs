...
use crate::{
    ...
    ImplTraitLoweringMode, InferenceTable, Interner, Lifetime, LifetimeData, ParamLoweringMode,
    PlaceholderIndex, ProjectionTy, Substitution, TraitRef, Ty, TyKind, TyLoweringContext,
    VariableKinds,
...
// We return a list and not a hashmap because the number of RPITITs in a function should be small.
...
fn defaulted_impl_method_rpitit_values(
    ...
    let impl_generics = generics(db.upcast(), impl_id.into());
...
pub(crate) fn defaulted_trait_method_rpitit_values(
...
) -> EmptyOptimizedThinVec<(RpititTraitAssocTyId, Binders<Ty>)> {
    tracing::error!("defaulted_trait_method_rpitit_values()");
    let method_generics = generics(db.upcast(), method_id.into());
    let mut table = InferenceTable::new(db, db.trait_environment(method_id.into()));

    let data = db.function_data(method_id);
    let resolver = method_id.resolver(db.upcast());
    let mut ctx_ret = TyLoweringContext::new(db, &resolver, &data.types_map, method_id.into())
    ...
    // in an impl.
    let method_opaques_ret = ctx_ret.lower_ty(data.ret_type);
    tracing::error!("method_opaques_ret={method_opaques_ret:?}");
    let method_opaques_ret = table.normalize_associated_types_in(method_opaques_ret);
    tracing::error!("after normalization: method_opaques_ret={method_opaques_ret:?}");
    ...
    let method_assocs_ret = db
        ...
        .ret()
        .clone();
    tracing::error!("method_assocs_ret={method_assocs_ret:?}");
    let mut rpitit_to_infer_var_folder = RpititToInferVarFolder {
        db,
        table: &mut table,
    ...
    tracing::error!("after folding: method_assocs_ret={method_assocs_ret:?}");
    let trait_rpitit_to_infer_var = rpitit_to_infer_var_folder.trait_rpitit_to_infer_var;
    let method_assocs_ret = table.normalize_associated_types_in(method_assocs_ret);
    tracing::error!("after normalization: method_assocs_ret={method_assocs_ret:?}");
    ...
    EmptyOptimizedThinVec::from_iter(trait_rpitit_to_infer_var.into_iter().map(
        |(trait_assoc_id, infer_var)| {
            let trait_assoc = trait_assoc_id.loc(db);
