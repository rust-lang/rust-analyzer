...
pub fn hir_display_with_store<'a, T: HirDisplayWithExpressionStore + 'a>(
    ...
    ExpressionStoreMapAdapter(value, store)
}

struct ExpressionStoreMapAdapter<'a, T>(T, &'a ExpressionStore);

impl<'a, T> ExpressionStoreMapAdapter<'a, T> {
    fn wrap(types_map: &'a ExpressionStore) -> impl Fn(T) -> ExpressionStoreMapAdapter<'a, T> {
        move |value| ExpressionStoreMapAdapter(value, types_map)
    }
}

impl<T: HirDisplayWithExpressionStore> HirDisplay for ExpressionStoreMapAdapter<'_, T> {
    fn hir_fmt(&self, f: &mut HirFormatter<'_>) -> Result<(), HirDisplayError> {
...
impl HirDisplayWithExpressionStore for TypeRefId {
    fn hir_fmt(
        ...
        match &store[*self] {
            ...
            TypeRef::Tuple(elems) => {
                write!(f, "(")?;
                f.write_joined(elems.iter().map(ExpressionStoreMapAdapter::wrap(store)), ", ")?;
                if elems.len() == 1 {
                    write!(f, ",")?;
                }
                write!(f, ")")?;
            ...
            TypeRef::ImplTrait(bounds) => {
                write!(f, "impl ")?;
                f.write_joined(bounds.iter().map(ExpressionStoreMapAdapter::wrap(store)), " + ")?;
            }
