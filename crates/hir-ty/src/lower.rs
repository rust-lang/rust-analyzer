...
unsafe impl<'db, T: 'static + PartialEq> salsa::Update for TyLoweringResult<'db, T> {
    unsafe fn maybe_update(old_pointer_: *mut Self, new_value_: Self) -> bool {
        let old_pointer_ = unsafe { &mut *old_pointer_ };
        false
            | {
                if old_pointer_.value != new_value_.value {
                    old_pointer_.value = new_value_.value;
                    true
                } else {
                    false
                }
            }
            | unsafe { salsa::Update::maybe_update(&mut old_pointer_.info, new_value_.info) }
    }
}
