...
use crate::{
    ...
    next_solver::{
        DbInterner, ParamEnvAnd, SolverDefIds,
        infer::{
            DbInternerInferExt, InferCtxt, canonical::canonicalizer::OriginalQueryValues,
            snapshot::CombinedSnapshot,
        },
        mapping::{ChalkToNextSolver, InferenceVarExt},
    },
    to_chalk_trait_id,
    traits::{
        FnTrait, NextTraitSolveResult, next_trait_solve, next_trait_solve_canonical_in_ctxt,
...
impl<'a> InferenceTable<'a> {
    ...
    pub(crate) fn canonicalize_with_free_vars<T>(
        &mut self,
        t: ParamEnvAnd<'a, T>,
    ) -> (rustc_type_ir::Canonical<DbInterner<'a>, ParamEnvAnd<'a, T>>, OriginalQueryValues<'a>)
    where
    ...
    pub(crate) fn try_unify<T: ChalkToNextSolver<'a, U>, U: Relate<DbInterner<'a>>>(
        ...
        match self.infer_ctxt.relate(param_env, lhs, variance, rhs, span) {
            Ok(res) => {
                let goals = res
                    .into_iter()
                    .map(|g| ChalkToNextSolver::from_nextsolver(g, self.interner))
                    .collect();
    ...
    pub(crate) fn resolve_ty_shallow(&mut self, ty: &Ty) -> Ty {
        ...
        ChalkToNextSolver::from_nextsolver(
            self.infer_ctxt.resolve_vars_if_possible(ty.to_nextsolver(self.interner)),
            self.interner,
        )
    }
    ...
    fn unify_opaque_instead_of_solve(
        &mut self,
    ...
    pub(crate) fn resolve_obligations_as_possible(&mut self) {
        ...
        while mem::take(&mut changed) {
            ...
            for goal in obligations.drain(..) {
                ...
                let result =
                    next_trait_solve_in_ctxt(&self.infer_ctxt, goal.to_nextsolver(self.interner));
        ...
        F: Fn(InferenceVar, VariableKind, GenericArg, DebruijnIndex) -> GenericArg,
    {
            ...
            match kind {
                TyVariableKind::General => {
                    ...
                    if let Ok(known_ty) = self.table.infer_ctxt.probe_ty_var(vid) {
                        let known_ty: Ty =
                            ChalkToNextSolver::from_nextsolver(known_ty, self.table.interner);
        ...
        fn fold_inference_const(
            ...
            if let Ok(known_const) = self.table.infer_ctxt.probe_const_var(vid) {
                let known_const: Const =
                    ChalkToNextSolver::from_nextsolver(known_const, self.table.interner);
                // known_ty may contain other variables that are known by now
