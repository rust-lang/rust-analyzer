...
fn check_doc_test(assist_id: &str, before: &str, after: &str) {
    ...
    let before = db.file_text(file_id.file(&db)).text(&db).to_string();
...
fn check_with_config(
    ...
    let file_with_caret_id = sema
        .attach_first_edition_opt(file_with_caret_id.file(&db))
        .unwrap_or(file_with_caret_id);
    let text_without_caret = db.file_text(file_with_caret_id.file(&db)).text(&db).to_string();
    ...
    match (assist, expected) {
        (Some(assist), ExpectedResult::After(after)) => {
            ...
            // Sort by file ID for deterministic output (IDs are assigned in fixture definition order)
            let mut source_file_edits: Vec<_> = source_change.source_file_edits.into_iter().collect();
            source_file_edits.sort_by_key(|(file, _)| *file);
            for (file_id, (edit, snippet_edit)) in source_file_edits {
