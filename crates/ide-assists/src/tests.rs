...
fn check_doc_test(assist_id: &str, before: &str, after: &str) {
    ...
    let frange = ide_db::FileRange { file_id: file_id.file_id(), range: selection.into() };
...
fn check_with_config(
    ...
    let file_with_caret_id =
        sema.attach_first_edition_opt(file_with_caret_id.file_id()).unwrap_or(file_with_caret_id);
    let text_without_caret = db.file_text(file_with_caret_id.file_id()).text(&db).to_string();
...
fn assist_order_field_struct() {
    ...
    let frange =
        FileRange { file_id: file_id.file_id(), range: TextRange::empty(before_cursor_pos) };
...
"#,
    ...
    let assists = assists(
        ...
        FileRange { file_id: frange.file_id.file_id(), range: frange.range },
    );
