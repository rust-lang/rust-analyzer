    ...
    acc.add(
        ...
        |builder| {
            ...
            for item in impl_.get_or_create_assoc_item_list().assoc_items() {
                process_assoc_item(&item, param_name);
            }
...
fn generic_param_name(trait_: &ast::Trait) -> SmolStr {
    let mut names_gen = NameGenerator::new_with_names([].into_iter());
...
fn process_assoc_item(origin: &ast::AssocItem, param_name: &str) {
    let Some(item) =
    ...
    match &item {
        ...
        Right(right) => {
            ...
            match right {
                Left(type_item) => process_type_alias(path, type_item),
                Right(const_item) => process_const_item(path, const_item),
            }
        }
    }
}
...
fn process_method(f: &ast::Fn) {
    let body = f.get_or_create_body();
    let method = f
        .name()
    ...
    let arg_list = generate_arg_list(f.param_list());
...
fn process_type_alias(path: ast::Path, type_item: &ast::TypeAlias) {
    ...
    {
        ted::insert(ted::Position::after(&token), path.syntax());
        ted::insert(ted::Position::after(&token), make::token(T![=]));
    } else if let Some(ty) = type_item.ty() {
        ted::replace(ty.syntax(), path.syntax());
    }
}

fn process_const_item(path: ast::Path, const_item: &ast::Const) {
    if let Some(ty) = const_item.ty()
        && const_item.eq_token().is_none()
    {
        ted::insert(ted::Position::after(ty.syntax()), path.syntax());
        ted::insert(ted::Position::after(ty.syntax()), make::token(T![=]));
    } else if let Some(init) = const_item.body() {
        ted::replace(init.syntax(), path.syntax());
    }
}

fn generate_arg_list(params: Option<ast::ParamList>) -> ast::ArgList {
    ...
    let args = params
        ...
        .map(|it| match it {
            ...
            (Some(pat), None) => {
                ...
                ted::replace(pat.syntax(), arg.clone_for_update().syntax());
                arg
            }
