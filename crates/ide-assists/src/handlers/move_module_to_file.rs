...
pub(crate) fn move_module_to_file(acc: &mut Assists, ctx: &AssistContext<'_, '_>) -> Option<()> {
    ...
    let l_curly_offset = module_items.syntax().text_range_without_outer_trivia().start();
    if l_curly_offset <= ctx.offset() {
        cov_mark::hit!(available_before_curly);
        return None;
    }
    let target = TextRange::new(
        module_ast.syntax().text_range_without_outer_trivia().start(),
        l_curly_offset,
    );
    ...
    acc.add(
        ...
        |builder| {
            ...
            let replacement_start = match module_ast.mod_token() {
                ...
                None => module_ast.syntax().text_range_without_outer_trivia(),
            }
            .start();
            ...
            builder.replace(
                TextRange::new(
                    replacement_start,
                    module_ast.syntax().text_range_without_outer_trivia().end(),
                ),
                buf,
            );
