...
pub(crate) fn move_to_mod_rs(acc: &mut Assists, ctx: &AssistContext<'_, '_>) -> Option<()> {
    ...
    let trimmed_file_range =
        trimmed_text_range(&source_file, source_file.syntax().text_range_without_outer_trivia());
    ...
    let target = source_file.syntax().text_range_without_outer_trivia();
