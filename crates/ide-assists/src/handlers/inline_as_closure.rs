...
pub(crate) fn inline_as_closure(acc: &mut Assists, ctx: &AssistContext<'_>) -> Option<()> {
    ...
    rename_usages(func, &body, &path, ctx.file_id(), sema)?;
...
fn rename_usages(
    ...
    if let Some(generic_arg_list) = path.path()?.segment()?.generic_arg_list() {
        if let Some((target, source)) = sema.scope(path.syntax()).zip(sema.scope(body.syntax())) {
            PathTransform::function_call(&target, &source, func, generic_arg_list)
                .apply(body.syntax());
        }
    }
