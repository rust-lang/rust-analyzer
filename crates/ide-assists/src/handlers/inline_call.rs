...
use syntax::{
    AstNode, NodeOrToken, SyntaxKind, T, TextRange,
    ast::{
...
fn inline<'db>(
    ...
    // Do not use original range as the body may be generated by a macro without mapping
    ...
    let usages_for_locals = |local| {
        let body_root = fn_body.syntax().tree_top();
        Definition::Local(local)
            ...
            .unwrap_or_default()
            .into_iter()
            .filter(move |it| it.name.syntax().tree_top() == body_root)
    };
    ...
    let rewrite_self_to_this = |editor: &SyntaxEditor| {
        ...
        // Due to the difficulty in handling the parameters referenced by macro calls
        // in macro generation functions, so add a syntax-based fallback
        let mut depth = 0u32;
        let not_replaced = |it: &syntax::SyntaxElement| {
            self_token_usages
                .iter()
                .all(|usage| !usage.text_range().contains_range(it.text_range()))
        };
        let is_item_like = |kind| ast::Item::can_cast(kind) && !ast::MacroCall::can_cast(kind);
        for event in body.syntax().preorder_with_tokens() {
            match event {
                syntax::WalkEvent::Enter(it) if is_item_like(it.kind()) => depth += 1,
                syntax::WalkEvent::Leave(it) if is_item_like(it.kind()) => depth -= 1,
                syntax::WalkEvent::Enter(it)
                    if it.kind() == T![self] && depth == 0 && not_replaced(&it) =>
                {
                    editor.replace(it, this_token.clone());
                }
                _ => (),
            }
        }
    ...
    fn reindent<T: AstNodeEdit>(value: T) -> T {
        value.reset_indent().indent(1.into())
    }
    let is_async_fn = function.is_async(sema.db);
    if is_async_fn {
        cov_mark::hit!(inline_call_async_fn);
        body = make
            .async_move_block_expr(body.statements().map(reindent), body.tail_expr().map(reindent));
        ...
        let stmts: Vec<ast::Stmt> =
            let_stmts.into_iter().chain(body.statements().map(reindent)).collect();
        body = make.block_expr(stmts, body.tail_expr().map(reindent));
        original_body_indent = IndentLevel(0);
    }
