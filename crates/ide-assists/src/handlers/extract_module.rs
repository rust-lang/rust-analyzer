...
impl Module {
                            ...
                            .insert(use_.syntax().text_range_without_outer_trivia().start())
                    {
                        ...
                        let replacements: Vec<_> = {
                            ...
                            edit_root
                                ...
                                .filter(|seg| {
                                    seg.syntax().text_without_outer_trivia()
                                        == name_ref.syntax().text_without_outer_trivia()
                                })
                                .filter_map(|seg| {
                                    Some((
                                        seg.syntax().parent()?,
    ...
    fn change_visibility(&mut self, record_fields: Vec<SyntaxNode>, make: &SyntaxFactory) {
        ...
        for (_, field_owner) in record_field_parents {
            for desc in field_owner.descendants().filter_map(ast::RecordField::cast) {
                let is_record_field_present = record_fields.iter().any(|x| {
                    x.text_without_outer_trivia() == desc.syntax().text_without_outer_trivia()
                });
            ...
            let insert_targets: Vec<_> = replacements
                ...
                .filter_map(|(_, syntax)| {
                    syntax
                        ...
                        .find(|nt| !matches!(nt.kind(), SyntaxKind::DOC_COMMENT | SyntaxKind::ATTR))
                })
                .collect();
    ...
    fn process_use_stmt_for_import_resolve(
        ...
        for path_seg in use_stmt.syntax().descendants().filter_map(ast::PathSegment::cast) {
            if path_seg.syntax().text_without_outer_trivia()
                == node_syntax.text_without_outer_trivia()
            {
                ...
                for use_tree in path_seg.syntax().ancestors().filter_map(ast::UseTree::cast) {
                    if use_tree.syntax().text_without_outer_trivia()
                        == node_syntax.text_without_outer_trivia()
                    {
