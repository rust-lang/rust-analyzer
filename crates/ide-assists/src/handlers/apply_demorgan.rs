        ...
        if let BinExpr(bin_expr) = &expr {
            if let BinExpr(cbin_expr) = &demorganed {
                if op == bin_expr.op_kind()? {
                    ...
                    if needs_parens_in_place_of(&inv, prec) {
...
fn needs_parens_in_place_of(this: &ast::Expr, prec: ExprPrecedence) -> bool {
    let this = precedence(&this);
    this.needs_parentheses_in(prec)
}
