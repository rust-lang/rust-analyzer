...
fn test_where_predicates() {
    fn assert_bound(text: &str, bound: Option<TypeBound>) {
        assert_eq!(text, bound.unwrap().to_string());
    }
...
{}
    ...
    assert_eq!("T", pred.ty().unwrap().to_string());
    ...
    assert_eq!("Iterator::Item", pred.ty().unwrap().to_string());
    ...
    assert_eq!("<T as Iterator>::Item", pred.ty().unwrap().to_string());
    assert_bound("Debug", bounds.next());
    assert_bound("'a", bounds.next());
    ...
    assert_eq!("F", pred.ty().unwrap().to_string());
