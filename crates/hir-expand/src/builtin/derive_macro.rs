...
struct AdtParam {
    ...
    const_ty: Option<tt::Subtree>,
    bounds: Option<tt::Subtree>,
}
...
fn expand_simple_derive(
    ...
    ExpandResult::ok(expand_simple_derive_with_parsed(
        ...
        make_trait_body,
        true,
        tt::Subtree::empty(tt::DelimSpan::from_single(invoc_span)),
    ))
}
...
fn coerce_pointee_expand(span: Span, tt: &tt::Subtree) -> ExpandResult<tt::Subtree> {
    let (adt, _span_map) = match to_adt_syntax(tt, span) {
        Ok(it) => it,
        Err(err) => {
            return ExpandResult::new(tt::Subtree::empty(tt::DelimSpan::from_single(span)), err);
        }
    };
    let adt = adt.clone_for_update();
    let ast::Adt::Struct(strukt) = &adt else {
        return ExpandResult::new(
            tt::Subtree::empty(tt::DelimSpan::from_single(span)),
