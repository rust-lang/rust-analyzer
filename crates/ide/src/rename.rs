...
use hir::{AsAssocItem, HasContainer, HirDisplay, ImportPathConfig, InFile, Name, Semantics, sym};
use ide_db::{
...
fn transform_method_call_into_assoc_fn(
    ...
    source_change: &mut SourceChange,
    f: hir::Function,
    import_path_config: ImportPathConfig,
) {
    let calls = Definition::Function(f).usages(sema).all();
    for (_file_id, calls) in calls {
        for call in calls {
...
fn rename_self_to_param(
    ...
    transform_method_call_into_assoc_fn(sema, &mut source_change, fn_def, import_path_config);
    Ok(source_change)
}
