...
fn on_left_brace_typed(reparsed: &SourceFile, offset: TextSize) -> Option<TextEdit> {
    ...
    if segment.syntax().text_range_without_outer_trivia().start() != offset {
        return None;
    }
    ...
    Some(TextEdit::insert(
        tree.syntax().text_range_without_outer_trivia().end() + TextSize::of("{"),
        "}".to_owned(),
    ))
}
...
fn on_delimited_node_typed(
    ...
    let (filter, node) = t
        .parent_ancestors()
        .take_while(|n| n.text_range_without_outer_trivia().start() == offset)
        .find_map(|n| kinds.iter().find(|&kind_filter| kind_filter(n.kind())).zip(Some(n)))?;
    let mut node = node
        .ancestors()
        .take_while(|n| n.text_range_without_outer_trivia().start() == offset && filter(n.kind()))
        .last()?;
        ...
        let all_prev_sib_attr = {
            ...
            loop {
                match node.prev_sibling() {
                    Some(sib) if sib.kind() == SyntaxKind::ATTR => node = sib,
                    Some(_) => break false,
                    None => break true,
                };
            }
    ...
    Some(TextEdit::insert(
        node.text_range_without_outer_trivia().end() + TextSize::of(opening_bracket),
...
fn on_eq_typed(file: &SourceFile, offset: TextSize) -> Option<TextEdit> {
    ...
    fn assign_expr(file: &SourceFile, offset: TextSize) -> Option<TextEdit> {
        ...
        let offset = expr.syntax().text_range_without_outer_trivia().end();
    ...
    fn let_stmt(file: &SourceFile, offset: TextSize) -> Option<TextEdit> {
        ...
        let offset = let_stmt.syntax().text_range_without_outer_trivia().end();
        Some(TextEdit::insert(offset, ";".to_owned()))
    }
}
...
fn on_dot_typed(file: &SourceFile, offset: TextSize) -> Option<TextEdit> {
    let blank = file.syntax().token_at_offset(offset).left_biased()?;
    if !matches!(blank.kind(), SyntaxKind::WHITESPACE | SyntaxKind::NEWLINE) {
        return None;
    }

    let run = syntax::algo::blank_run(&blank).range;
    let run = file.syntax().text().slice(TextRange::new(run.start(), offset)).to_string();

    // if prior is fn call over multiple lines dont indent
    // or if previous is method call over multiples lines keep that indent
    let (_prefix, current_indent) = run.rsplit_once('\n')?;
    let current_indent_len = TextSize::of(current_indent);

    let parent = blank.owning_node()?;
    ...
    let receiver_is_multiline =
        receiver.syntax().text_without_outer_trivia().find_char('\n').is_some();
...
fn on_plus_typed(file: &SourceFile, offset: TextSize) -> Option<TextEdit> {
    ...
    if ast::RefType::can_cast(kind) || ast::PtrType::can_cast(kind) || ast::RetType::can_cast(kind)
    {
        ...
        builder
            .insert(trait_type.syntax().text_range_without_outer_trivia().start(), "(".to_owned());
        builder.insert(trait_type.syntax().text_range_without_outer_trivia().end(), ")".to_owned());
        Some(builder.finish())
    } else {
        None
    }
