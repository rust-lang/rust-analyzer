...
fn parse_rust_string(token: SyntaxToken) -> Option<String> {
    ...
    // SAFETY: This is actually just a modified version of str::replace that
    // tracks replacements, so I defer to the standard library on safety here
    ...
    for (start, part) in text.match_indices("$0") {
        ...
        trim_result.push_str(unsafe { text.get_unchecked(last_end..start) });
        skipped += part.len();
        last_end = start + part.len();
    }
    trim_result.push_str(unsafe { text.get_unchecked(last_end..text.len()) });
