...
use either::Either;
use hir_def::{
    AdtId, AssocItemId, DefWithBodyId, DoNotComplete, ExternCrateId, HasModule, ImplId, Lookup,
    MacroId, ModuleDefId, ModuleId, TraitId,
    db::DefDatabase,
...
pub struct FileSymbol {
    ...
    pub is_alias: bool,
    pub is_assoc: bool,
    pub do_not_complete: DoNotComplete,
}
...
impl<'a> SymbolCollector<'a> {
    ...
    fn collect_from_module(&mut self, module_id: ModuleId) {
        ...
        let mut push_import = |this: &mut Self, i: ImportId, name: &Name, def: ModuleDefId, vis| {
            ...
            this.symbols.insert(FileSymbol {
                ...
                is_alias: false,
                is_assoc: false,
                do_not_complete: DoNotComplete::Complete,
            });
        };
    ...
    fn collect_from_trait(&mut self, trait_id: TraitId, trait_do_not_complete: DoNotComplete) {
    ...
    fn push_assoc_item(
        ...
        name: &Name,
        trait_do_not_complete: Option<DoNotComplete>,
    ) {
        match assoc_item_id {
        ...
        <<L as Lookup>::Data as HasSource>::Value: HasName,
    {
        ...
        let Some(name_node) = source.value.name() else { return DoNotComplete::Complete };
        ...
        if let Some(attrs) = def.attrs(self.db) {
            do_not_complete = DoNotComplete::extract(matches!(def, ModuleDef::Trait(_)), &attrs);
            if let Some(trait_do_not_complete) = trait_do_not_complete {
                do_not_complete =
                    DoNotComplete::for_trait_item(trait_do_not_complete, do_not_complete);
            }
