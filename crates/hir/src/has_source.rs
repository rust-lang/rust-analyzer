...
impl HasSource for Adt {
    ...
    fn source(self, db: &dyn HirDatabase) -> InFile<Self::Ast> {
        match self {
            Adt::Struct(s) => s.source(db).map(ast::Adt::Struct),
            Adt::Union(u) => u.source(db).map(ast::Adt::Union),
            Adt::Enum(e) => e.source(db).map(ast::Adt::Enum),
        }
    }
}
...
impl HasSource for Macro {
    ...
    fn source(self, db: &dyn HirDatabase) -> InFile<Self::Ast> {
        match self.id {
            MacroId::Macro2Id(it) => it
                .lookup(db.upcast())
                .source(db.upcast())
                .map(ast::Macro::MacroDef)
                .map(Either::Left),

            MacroId::MacroRulesId(it) => it
                .lookup(db.upcast())
                .source(db.upcast())
            ...
            MacroId::ProcMacroId(it) => {
                it.lookup(db.upcast()).source(db.upcast()).map(Either::Right)
            }
        }
    }
}
...
impl HasSource for LocalSource {
    ...
    fn source(self, _: &dyn HirDatabase) -> InFile<Self::Ast> {
        self.source
    }
}
