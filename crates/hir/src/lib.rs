...
impl Crate {
    ...
    ) -> impl Iterator<Item = (Either<ModuleDef, Macro>, DoNotComplete)> {
...
impl Trait {
    ...
    /// `#[rust_analyzer::do_not_complete(...)]` mode.
    pub fn do_not_complete(self, db: &dyn HirDatabase) -> DoNotComplete {
        DoNotComplete::extract(true, &self.attrs(db))
    }
}
