...
impl TupleField {
    ...
    pub fn ty<'db>(&self, db: &'db dyn HirDatabase) -> Type<'db> {
        ...
        Type::new_body(db, self.owner, ty)
    }
}
...
impl Field {
    ...
    pub fn ty<'db>(&self, db: &'db dyn HirDatabase) -> Type<'db> {
        ...
        let ty = db.field_types(var_id)[self.id].get().instantiate_identity();
...
impl TypeParam {
    ...
    pub fn trait_bounds(self, db: &dyn HirDatabase) -> Vec<Trait> {
        let self_ty = self.ty(db).ty.instantiate_identity();
...
impl<'db> Closure<'db> {
    ...
    pub fn captured_items(&self, db: &'db dyn HirDatabase) -> Vec<ClosureCapture<'db>> {
        ...
        infer.closures_data[&closure]
            ...
            .values()
            .flatten()
            .map(|capture| ClosureCapture { owner, closure, capture })
            .collect()
    }
...
impl_from!(GenericDefId, BuiltinDeriveImplId for TypeOwnerId);
...
impl<'db> Type<'db> {
    ...
    fn from_value_def(db: &'db dyn HirDatabase, def: impl Into<ValueTyDefId>) -> Self {
        ...
        Type::new(def, ty.instantiate_identity())
    }
    ...
    pub fn instantiate_with_errors(&self) -> Self {
        ...
        Type::no_params(krate, self.ty.instantiate(interner, args))
    }
    ...
    pub fn instantiate(&self, args: impl IntoIterator<Item: Borrow<Type<'db>>>) -> Type<'db> {
        ...
        Type { owner, ty: EarlyBinder::bind(self.ty.instantiate(interner, args)) }
    }
    ...
    fn instantiate_many_with_infer(
        ...
        tys.into_iter().map(move |ty| {
            ...
            ty.ty.instantiate(infcx.interner, args)
        })
    }
    ...
    pub fn new_tuple(tys: impl IntoIterator<Item: Borrow<Type<'db>>>) -> Self {
        let interner = DbInterner::conjure();
    ...
    pub fn contains_reference(&self, db: &'db dyn HirDatabase) -> bool {
        let interner = DbInterner::new_no_crate(db);
        return self.ty.instantiate_identity().visit_with(&mut Visitor { interner }).is_break();
    ...
    pub fn into_future_output(&self, db: &'db dyn HirDatabase) -> Option<Type<'db>> {
        ...
        if !traits::implements_trait_unique(self.ty.instantiate_identity(), db, env, trait_) {
            return None;
        }
    ...
    pub fn impls_iterator(self, db: &'db dyn HirDatabase) -> bool {
        ...
        traits::implements_trait_unique(self.ty.instantiate_identity(), db, env, iterator_trait)
    }
    ...
    pub fn impls_fnonce(&self, db: &'db dyn HirDatabase) -> bool {
        ...
        traits::implements_trait_unique(self.ty.instantiate_identity(), db, env, fnonce_trait)
    }
