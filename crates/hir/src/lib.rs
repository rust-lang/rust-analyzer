...
use hir_def::{
    ...
    FunctionId, GenericDefId, GenericParamId, HasModule, ImplId, InternedModuleId, ItemContainerId,
        ...
        for (pat_or_expr, mismatch) in infer.type_mismatches() {
            ...
            acc.push(
                TypeMismatch {
                    ...
                    actual: Type::new(db, id, mismatch.actual),
                }
                .into(),
            );
        }
    ...
    pub fn ty(self, db: &dyn HirDatabase) -> Type<'_> {
        match self.id {
            ...
            AnyFunctionId::BuiltinDeriveImplMethod { method, impl_ } => {
                ...
                let krate = impl_.module(db).krate();
...
impl SelfParam {
    ...
    pub fn ty<'db>(&self, db: &'db dyn HirDatabase) -> Type<'db> {
        ...
        Type { env, ty: sig.skip_binder().inputs().get(0).unwrap() }
    }
...
impl Impl {
    ...
    pub fn all_for_type<'db>(
        ...
        extend_with_impls(Either::Left(method_resolution::incoherent_inherent_impls(
            db,
            simplified_ty,
        )));
    ...
    pub fn trait_(self, db: &dyn HirDatabase) -> Option<Trait> {
        match self.id {
            ...
            AnyImplId::BuiltinDeriveImplId(id) => {
                let loc = id.loc(db);
                let lang_items = hir_def::lang_item::lang_items(db, loc.adt.module(db).krate());
