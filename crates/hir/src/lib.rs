        ...
        if let Ok(borrowck_results) = db.borrowck(self.into()) {
            for borrowck_result in borrowck_results.iter() {
                ...
                for moof in &borrowck_result.moved_out_of_ref {
                    ...
                    acc.push(
                        MovedOutOfRef { ty: Type::new_for_crate(db, krate, moof.ty.clone()), span }
                            .into(),
                    )
                }
...
impl Function {
    ...
    pub fn assoc_fn_params(self, db: &dyn HirDatabase) -> Vec<Param<'_>> {
        ...
        callable_sig
            ...
            .enumerate()
            .map(|(idx, ty)| {
                let ty = Type { env: environment, ty: ty.clone() };
    ...
    pub fn returns_impl_future(self, db: &dyn HirDatabase) -> bool {
        ...
        let Some(future_trait_id) =
            db.lang_item(self.ty(db).env.krate(db), LangItem::Future).and_then(|t| t.as_trait())
        else {
            return false;
        };
        let Some(sized_trait_id) =
            db.lang_item(self.ty(db).env.krate(db), LangItem::Sized).and_then(|t| t.as_trait())
        else {
            return false;
        };
...
impl BuiltinType {
    ...
    pub fn ty<'db>(self, db: &'db dyn HirDatabase) -> Type<'db> {
        ...
        Type::new_for_crate(db, core, TyBuilder::builtin(self.inner))
    }
...
impl<'db> GenericSubstitution<'db> {
    fn new(def: GenericDefId, subst: Substitution, env: TraitEnvironment<'db>) -> Self {
        Self { def, subst, env }
    }
...
impl Impl {
    ...
    pub fn all_for_type<'db>(db: &'db dyn HirDatabase, Type { ty, env }: Type<'db>) -> Vec<Impl> {
        let def_crates = match method_resolution::def_crates(db, &ty, env.krate(db)) {
...
impl<'db> TraitRef<'db> {
    pub(crate) fn new_with_resolver(
        ...
        let env = resolver.generic_def().map_or_else(
            || TraitEnvironment::empty(db, resolver.krate()),
...
impl<'db> Type<'db> {
    pub(crate) fn new_with_resolver(db: &'db dyn HirDatabase, resolver: &Resolver, ty: Ty) -> Self {
        Type::new_with_resolver_inner(db, resolver, ty)
    }
    ...
    pub(crate) fn new_for_crate(db: &'db dyn HirDatabase, krate: base_db::Crate, ty: Ty) -> Self {
        Type { env: TraitEnvironment::empty(db, krate), ty }
    }
    ...
    pub fn new_tuple(db: &'db dyn HirDatabase, krate: base_db::Crate, tys: &[Self]) -> Self {
        ...
        Type { env: TraitEnvironment::empty(db, krate), ty: TyBuilder::tuple_with(tys) }
    }
    ...
    pub fn contains_reference(&self, db: &'db dyn HirDatabase) -> bool {
        return go(db, self.env.krate(db), &self.ty);
    ...
    pub fn into_future_output(&self, db: &'db dyn HirDatabase) -> Option<Type<'db>> {
        let trait_ = db
            .lang_item(self.env.krate(db), LangItem::IntoFutureIntoFuture)
            .and_then(|it| {
            ...
            .or_else(|| {
                let future_trait = db.lang_item(self.env.krate(db), LangItem::Future)?;
    ...
    pub fn future_output(self, db: &'db dyn HirDatabase) -> Option<Type<'db>> {
        let future_output =
            db.lang_item(self.env.krate(db), LangItem::FutureOutput)?.as_type_alias()?;
    ...
    pub fn iterator_item(self, db: &'db dyn HirDatabase) -> Option<Type<'db>> {
        let iterator_trait = db.lang_item(self.env.krate(db), LangItem::Iterator)?.as_trait()?;
    ...
    pub fn impls_iterator(self, db: &'db dyn HirDatabase) -> bool {
        let Some(iterator_trait) =
            db.lang_item(self.env.krate(db), LangItem::Iterator).and_then(|it| it.as_trait())
        else {
            return false;
        };
        let canonical_ty =
    ...
    pub fn into_iterator_iter(self, db: &'db dyn HirDatabase) -> Option<Type<'db>> {
        let trait_ =
            db.lang_item(self.env.krate(db), LangItem::IntoIterIntoIter).and_then(|it| {
    ...
    pub fn impls_fnonce(&self, db: &'db dyn HirDatabase) -> bool {
        let fnonce_trait = match FnTrait::FnOnce.get_id(db, self.env.krate(db)) {
            Some(it) => it,
            None => return false,
        };
    ...
    pub fn impls_trait(&self, db: &'db dyn HirDatabase, trait_: Trait, args: &[Type<'db>]) -> bool {
        ...
        let goal = Canonical {
            value: hir_ty::InEnvironment::new(self.env.env(db), trait_ref.cast(Interner)),
            binders: CanonicalVarKinds::empty(Interner),
        };

        db.trait_solve(self.env.krate(db), self.env.block(db), goal).is_some()
    }
    ...
    pub fn normalize_trait_assoc_type(
        ...
        let ty = db.normalize_projection(projection, self.env);
        if ty.is_unknown() { None } else { Some(self.derived(ty)) }
    }

    pub fn is_copy(&self, db: &'db dyn HirDatabase) -> bool {
        let lang_item = db.lang_item(self.env.krate(db), LangItem::Copy);
    ...
    pub fn as_callable(&self, db: &'db dyn HirDatabase) -> Option<Callable<'db>> {
        let callee = match self.ty.kind(Interner) {
            ...
            kind => {
                ...
                let (fn_trait, sig) = hir_ty::callable_sig_from_fn_trait(ty, self.env, db)?;
                return Some(Callable {
                    ty: self.clone(),
                    sig,
    ...
    pub fn iterate_method_candidates_split_inherent(
        ...
        let environment = scope
            .resolver()
            .generic_def()
            .map_or_else(|| TraitEnvironment::empty(db, krate.id), |d| db.trait_environment(d));
    ...
    pub fn env_traits<'a>(&'a self, db: &'a dyn HirDatabase) -> impl Iterator<Item = Trait> + 'a {
        ...
        self.autoderef_(db)
            ...
            .flat_map(|ty| {
                self.env
                    .traits_in_scope_from_clauses(db, ty)
    ...
    pub fn layout(&self, db: &'db dyn HirDatabase) -> Result<Layout, LayoutError> {
        db.layout_of_ty(self.ty.clone(), self.env)
            .map(|layout| Layout(layout, db.target_data_layout(self.env.krate(db)).unwrap()))
    }

    pub fn drop_glue(&self, db: &'db dyn HirDatabase) -> DropGlue {
        db.has_drop_glue(self.ty.clone(), self.env)
    }
}
