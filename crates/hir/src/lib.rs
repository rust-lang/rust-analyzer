...
use hir_def::{
    ...
    HasModule, ImplId, InternedModuleId, ItemContainerId, LifetimeParamId, LocalFieldId, Lookup,
    MacroExpander, MacroId, ModuleId, StaticId, StructId, SyntheticSyntax, TupleId, TypeAliasId,
    ...
    signatures::{EnumSignature, ImplFlags, StaticFlags, TraitFlags, VariantFields},
    src::HasSource as _,
...
impl Enum {
    ...
    pub fn variant_body_ty<'db>(self, db: &'db dyn HirDatabase) -> Type<'db> {
        Type::new_for_crate(
            ...
            TyBuilder::builtin(match EnumSignature::variant_body_type(db, self.id) {
...
impl Function {
    ...
    pub fn is_unsafe_to_call(
        ...
        let target_features =
            caller.map(|caller| hir_ty::TargetFeatures::from_fn(db, caller.id)).unwrap_or_default();
        matches!(
