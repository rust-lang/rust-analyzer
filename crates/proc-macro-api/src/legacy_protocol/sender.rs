//! Functions for the sender side, i.e. the rust-analyzer side.
...
pub(crate) fn expand(
    ...
    match response {
        Response::ExpandMacro(it) => Ok(it
            .map(|tree| {
                let mut expanded = FlatTree::to_subtree(tree, version, &span_data_table);
        ...
        Response::ExpandMacroExtended(it) => Ok(it
            .map(|resp| {
                let mut expanded = FlatTree::to_subtree(
                    resp.tree,
                    version,
