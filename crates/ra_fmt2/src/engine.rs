...
impl FmtDiff {
    ...
    fn apply_edit(&self, right_blk: &Block, ws: Whitespace, loc: SpaceLoc) {
        println!("{:?} {:?}", right_blk.as_element(), ws);
        match loc {
    ...
    fn correct_space(
        ...
        match rule.space.loc {
            ...
            SpaceLoc::After => {
                if !left_ws.match_space_after(rule.space)
                    && !right_ws.match_space_before(rule.space)
    ...
    pub(crate) fn spacing_diff(self, space_rules: &SpacingDsl) -> FmtDiff {
        ...
        for (left, right) in blocks {
            println!("{:?} {:?}", left.as_element(), right.as_element());
    ...
    fn check_indent(&self, anchor_set: &PatternSet<&Pattern>, block: &Block) {
        ...
        if block.get_indent() != anchors {
            ...
            // println!("INDENT {} CURR {:#?}", anchors, block);
        }
    }
