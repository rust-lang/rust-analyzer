...
use crate::dsl::{self, IndentDsl, IndentRule, IndentValue, SpaceLoc, SpacingDsl, SpacingRule};
...
impl FmtDiff {
    ...
    fn apply_edit(&self, right_blk: &Block, ws: Whitespace, loc: SpaceLoc) {
        match loc {
            ...
            // TODO is this not idempotent??
            SpaceLoc::Around => right_blk.set_spacing_around(ws),
        }
    }
    /// Builds the correct spacing based on the `SpacingRule` supplied.
    fn correct_space(
        &self,
        rule: &SpacingRule,
        left_blk: &Block,
        ...
        match rule.space.loc {
            SpaceLoc::Before => {
                if !right_ws.match_space_before(rule.space)
                    && rule.pattern.matches(right_blk.as_element())
                {
                    Some(Whitespace::from_rule(rule, left_blk, right_blk))
                } else {
                    None
                }
            }
...
pub(crate) fn format_pass(
    ...
    FmtDiff::new(fmt).spacing_diff(space_dsl).indent_diff(indent_dsl).into()
}

pub fn format_str(file: &str) -> Result<String, std::fmt::Error> {
