...
impl Block {
    ...
    pub(crate) fn eol_value(&self) -> bool {
        self.whitespace.borrow().space_kind() == SpaceValue::Newline
    }
    /// Returns true if `Block` starts with new line char.
    pub(crate) fn starts_with_lf(&self) -> bool {
        let ws = self.whitespace.borrow();
        ws.space_kind() == SpaceValue::Newline
        && ws.space_loc() == SpaceLoc::Before
    }
